...
public class SetStartTimeInventory {

    Main plugin = Main.plugin;
    private final Inventory inventory;
    private final Election election;

    public SetStartTimeInventory(Election election) {
        this.election = election;
        String title = colorize("&6Set start time");
        inventory = plugin.getServer().createInventory(null, 9, title);

        if (!election.usingStartTime()) { election.setStartTime(new Date()); }
    ...
    public void loadItems() {
        ...
        secondLore.add(colorize("&a") + election.getStartTime().toString());
        ...
        minuteLore.add(colorize("&a") + election.getStartTime().toString());
        ...
        hourLore.add(colorize("&a") + election.getStartTime().toString());
        ...
        dayLore.add(colorize("&a") + election.getStartTime().toString());
        ...
        weekLore.add(colorize("&a") + election.getStartTime().toString());
        ...
        yearLore.add(colorize("&a") + election.getStartTime().toString());
        ...
        disableMeta.setDisplayName(colorize("&4Disable Start Time"));
        List<String> disableLore = new ArrayList<>();
        disableLore.add(colorize("&cClick to disable Start Time"));
    ...
    public void open(Player player) {
        Main.openEditStartTimeInventories.put(player.getUniqueId(), this);
        player.openInventory(inventory);
    }

    public void increment(int seconds) {
        election.incrementStartTime(seconds);
        loadItems();
    }

    public void incrementYear(int year) {
        election.incrementStartTimeYear(year);
        loadItems();
    }

    public void disableTime() {
        election.removeStartTime();
    }
